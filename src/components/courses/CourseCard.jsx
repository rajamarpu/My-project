...
export default function CourseCard({ course, onViewDetails }) {
  ...
  const teacher =
    aiPersonalities.find((item) => item.id === course.aiTeacherId) ||
    aiPersonalities[Number(course.id?.replace(/\D/g, '') || 1) % aiPersonalities.length] ||
    aiPersonalities[0]
    ...
    >
      ...
      <div className="theme-dark relative overflow-hidden rounded-2xl border border-white/10 bg-slate-950/50">
        ...
        <div className="absolute inset-x-3 bottom-3 flex items-center justify-between gap-3">
          ...
          <span className="rounded-full border border-white/15 bg-slate-950/80 px-3 py-1 text-xs font-semibold text-amber-200 backdrop-blur">
            {course.badge || 'AI guided'}
          </span>
        </div>
      </div>

      <div className="relative mt-5 flex items-start gap-3">
        <img src={teacher.avatar} alt={`${teacher.name} avatar`} className="h-12 w-12 rounded-2xl border border-cyan-300/30 bg-slate-900 object-cover p-1" />
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-slate-300 dark:text-slate-300 light:text-slate-600">
            Guided by {teacher.name} - {teacher.teachingTone}
          </p>
        </div>
      </div>
      ...
      <div className="relative mt-5 grid gap-3 text-slate-300 sm:grid-cols-2">
        ...
        <span className="inline-flex items-center gap-2 text-sm text-cyan-300 dark:text-cyan-300 light:text-cyan-700">
          <Clock size={16} className="text-cyan-300 dark:text-cyan-300 light:text-cyan-700" /> {course.duration}
        </span>
        <span className="inline-flex items-center gap-2 text-sm text-green-300 dark:text-green-300 light:text-green-700">
          <Users size={16} className="text-green-300 dark:text-green-300 light:text-green-700" /> {course.enrolled} learners
        </span>
        <span className="inline-flex items-center gap-2 text-sm text-violet-300 dark:text-violet-300 light:text-violet-700">
          <Layers3 size={16} /> {course.curriculum?.length || 8} lessons
        </span>
        <span className="inline-flex items-center gap-2 text-sm text-slate-400 dark:text-slate-400 light:text-slate-500">
          <Heart
            size={16}
            className={cn('transition', active ? 'text-rose-400' : 'text-slate-500 dark:text-slate-500')}
          />
      ...
      <div className="relative mt-5">
        ...
        <div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-800/80">
          <div className="h-full rounded-full bg-gradient-to-r from-cyan-300 via-blue-400 to-fuchsia-400" style={{ width: `${progress}%` }} />
        </div>
      </div>
